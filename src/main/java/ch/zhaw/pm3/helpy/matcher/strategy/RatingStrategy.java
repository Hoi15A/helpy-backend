...
public class RatingStrategy extends MatcherStrategy {
    ...
    public Collection<User> getPotentialHelpers(Job job) {
        Comparator<User> comparator = (o1, o2) -> {
            double u1 = (double) o1.getRatings().stream().mapToInt(Integer::valueOf).sum() / o1.getRatings().size();
            double u2 = (double) o2.getRatings().stream().mapToInt(Integer::valueOf).sum() / o2.getRatings().size();
